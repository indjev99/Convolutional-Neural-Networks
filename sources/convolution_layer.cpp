...
inline unsigned int encode(unsigned int d, unsigned int w, unsigned int h, const Structure& s)
{
                ...
                for (unsigned int d2=0;d2<prevStructure.depth;++d2)
                {
                    for (unsigned int hOffset=0;hOffset<convolutionStructure.heigth;++hOffset)
                    {
                        for (unsigned int wOffset=0;wOffset<convolutionStructure.width;++wOffset)
                        {
                            int h2=h+hOffset;
                            int w2=w+wOffset;
                            int j=encode(d2,h2,w2,prevStructure);
                            int k=d*prevStructure.depth*convolutionStructure.size()+encode(d2,hOffset,wOffset,convolutionStructure);
